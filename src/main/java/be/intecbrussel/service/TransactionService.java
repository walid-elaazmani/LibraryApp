...
public class TransactionService {
    ...
    public double checkAccount(User user){
        ...
        for (BorrowedBook borrowedBook : bookToList) {
            if (borrowedBook.getDateOfReturn().isAfter(LocalDate.now()) && borrowedBook.getBook().getStatus() == Status.UNAVAILABLE){
                fineForBooks += (Math.abs(DAYS.between(borrowedBook.getDateOfReturn(), LocalDate.now()) * 0.20));
                System.out.println("You have a fine of: " + Math.abs(DAYS.between(borrowedBook.getDateOfReturn(), LocalDate.now()) * 0.20) + "€" + " for: " + borrowedBook.getBook().getTitle());
            }
        }
        return fineForBooks;
    }
